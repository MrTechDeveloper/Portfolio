...
export default class IntlPhoneInput extends React.Component {
  ...
  filterCountries = (value) => {
    ...
    const countryData = data.filter((obj) => (obj.en.indexOf(value) > -1 || obj.dialCode.indexOf(value) > -1));

    const newData = data.filter(item => {
      const itemData = `${item.en.toUpperCase()}, ${item.dialCode.toUpperCase()}`;
      ...
      <Modal animationType="slide" transparent={true} visible={this.state.modalVisible}>
        <SafeAreaView style={{
          ...
          <Icon
            ...
            solid
            style={{padding: 10}} />
            <Text style={{fontWeight:'bold'}}>Select Dialcode  </Text>
            <Text>  </Text>
          </View>
        </SafeAreaView>
      </Modal>
